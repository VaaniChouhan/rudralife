...
export default function BackgroundLayers() {
  return (
    <div id="bg-layers-container">
      ...
      <video id="bg-video-milind" className="fixed-bg-layer" muted playsInline loop preload="none" style={{ width: '100vw', height: '100vh', objectFit: 'cover' }}>
        <source data-src="/scene4.mp4" type="video/mp4" />
      </video>
      
      {/* S10: Vedic Energization (Scene 12: Havan fire) - Lazily Loaded */}
      <video id="bg-video-fire" className="fixed-bg-layer" muted playsInline loop preload="none" style={{ width: '100vw', height: '100vh', objectFit: 'cover' }}>
        <source data-src="/scene12.mp4" type="video/mp4" />
      </video>
      
      {/* S12, s-siddha-mala: Rarest Collection & Siddha Mala (Scene 14: Spotlight velvet) - Lazily Loaded */}
      <video id="bg-video-spotlight" className="fixed-bg-layer" muted playsInline loop preload="none" style={{ width: '100vw', height: '100vh', objectFit: 'cover' }}>
        <source data-src="/scene14.mp4" type="video/mp4" />
      </video>
      
      {/* S17: 25 Years of Trust (Scene 9: Banyan root) - Lazily Loaded */}
      <video id="bg-video-legacy" className="fixed-bg-layer" muted playsInline loop preload="none" style={{ width: '100vw', height: '100vh', objectFit: 'cover' }}>
        <source data-src="/scene9.mp4" type="video/mp4" />
      </video>
      
      {/* S24: Final CTA (Scene 25: Meditating horizon) - Lazily Loaded */}
      <video id="bg-video-final" className="fixed-bg-layer" muted playsInline loop preload="none" style={{ width: '100vw', height: '100vh', objectFit: 'cover' }}>
        <source data-src="/scene25.mp4" type="video/mp4" />
      </video>
      ...
      <div id="bg-img-yantra" className="fixed-bg-layer" data-bg-img="/copper_yantra.png" style={{ backgroundSize: 'cover', backgroundPosition: 'center' }}>
        <svg viewBox="0 0 100 100" style={{ position: 'absolute', top: '15%', left: '50%', transform: 'translateX(-50%)', width: '70%', opacity: 0.09, stroke: '#C49A3C', strokeWidth: 0.25, fill: 'none' }}>
          ...
          {[...Array(16)].map((_, i) => {
            ...
            return <path key={`p16-${i}`} d={`M${x1},${y1} Q${mx},${my} ${x2},${y2}`} strokeWidth="0.2" />;
          })}
          ...
          {[...Array(8)].map((_, i) => {
            ...
            const mx = 50 + rMid * Math.cos((midAngle * Math.PI) / 180);
            const my = 50 + rMid * Math.sin((midAngle * Math.PI) / 180);
            return <path key={`p8-${i}`} d={`M${x1},${y1} Q${mx},${my} ${x2},${y2}`} strokeWidth="0.25" />;
          })}
